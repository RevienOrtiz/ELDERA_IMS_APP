...
class SeniorController extends Controller
{
    ...
    public function index(Request $request): View
    {
        ...
        $pensionApplications = cache()->remember('pension_applications_page_1', 300, function () {
            return Application::select(['id', 'senior_id', 'status', 'submitted_at', 'created_at'])
                ->with(['senior:id,osca_id,first_name,last_name,middle_name,barangay,date_of_birth,sex'])
    ...
    public function updateAppAccount(Request $request, string $id)
    {
        ...
        if (!$senior->has_app_account || !$senior->user_id) {
            return redirect()->route('seniors')->with('error', 'Senior does not have an app account yet.');
        }
        ...
        // Get the user
        $user = \App\Models\User::findOrFail($senior->user_id);
    ...
    {
        // Get benefits applications only for seniors with complete data
        $benefitsApplications = Application::where('application_type', 'benefits')
            ->whereHas('senior', function($query) {
                // Only show applications for seniors with complete essential data
            ...
            ->with([
                ...
                //     $query->select('senior_id', 'milestone_age');
                // }
            ])
